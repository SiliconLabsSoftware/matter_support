...
sl_status_t sl_si91x_host_spi_transfer(const void *tx_buffer, void *rx_buffer, uint16_t buffer_length)
{
  ...
  // If the buffer length is less than the high speed transfer threshold, use blocking SPI transfer
  if (buffer_length < 16) {
    while (buffer_length) {
      *rx_buf = USART_SpiTransfer(SPI_HANDLE->initData.port, *tx_buf);
      if (tx_buffer != NULL)
        tx_buf++;
      if (rx_buffer != NULL)
        rx_buf++;
  ...
  if (osSemaphoreAcquire(transfer_done_semaphore, 1000) != osOK) {
    ...
    SILABS_LOG("ERR: SPI timed out %d/%d (rx%x rx%x)",
               itemsTransferred,
               itemsRemaining,
               (uint32_t)tx_buf,
               (uint32_t)rx_buf);
